...
因此本轮没有形成可用于堵转判定的实机电流，不能把自动停机硬件链路记为实机通过。用户确认现场没有电机、电池或电机功率电源，并接受以逻辑测试覆盖堵转判定与停机状态转换。只读诊断同时显示：
...
ADC 采集与发布没有报错，但现场条件无法产生非零 IPROPI 电流。纯逻辑测试已覆盖 300 ms 屏蔽、连续 100 ms 超限、无效/低电流样本复位、Tick 回绕、停止后守卫复位，以及确认堵转后写入 BRAKE 与 `OVERCURRENT=1` 的任务接线契约。未来有电机条件时，仍建议补做端到端电流链路测试：先确认空载约 0.5 A，再以可控方式验证触发与双向重启。
...
- 未执行 19 A 硬堵转
- 自动停机判定与状态转换：逻辑测试通过
- 自动停机端到端电流硬件链路：因现场无电机/功率电源未测试，作为后续建议项，不作为本次交付阻塞项
