...
- 判断只使用 NTC1/NTC2/NTC3，不使用当前未焊接的 BATT_NTC。输入与 `AT+SENSE?` 相同，来自最近五个完整成功采样周期的平均值，所以保护可能有最多约数秒的响应延迟。
- 任一路严格高于 60.0°C，或任一路转换为 `ERR`，都会锁存保护并停止 CHARGE/DRIVE、关闭 NMOS1/2、让电机进入 SLEEP；LED 不变。
