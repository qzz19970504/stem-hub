...
- 保护只使用 NTC1、NTC2、NTC3；当前硬件未焊接的 BATT_NTC 不参与判断。判断输入与 `AT+SENSE?` 显示一致，均为最近五个完整成功采样周期的平均结果，因此可能产生最多约数秒的响应延迟。
- 任一路严格高于 60.0°C，或任一路温度转换无效并显示 `ERR`，都会锁存过温保护。触发后取消 CHARGE/DRIVE，关闭 LM51770、MP4317、NMOS1、NMOS2，并让电机进入 SLEEP；LED 保持原状态。
